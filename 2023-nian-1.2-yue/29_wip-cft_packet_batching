...
在过去的30年里，我们使用的计算机增长速度之快令人难以想象。1995年 Alpha AXP 的论文谈到了机器的设计，它延续了前25年的趋势，速度提高了1000倍。

我们当然已经实现了这个目标，作为FreeBSD最初目标的386机器类似于我们今天在键盘中使用的微控制器。

即使有了这些变化，计算机性能的核心仍然保持不变，每单位工作执行的指令更少，而且速度会更快。这种网络化的基本事实导致了几种不同的方法来提高性能。我们已经研究了将工作从 CPU 转移到带有校验和卸载的网卡的机制。如果这种卡运行指令来校验输出的数据包，那么我们宝贵的 CPU 时间可以用来做其他事情。

校验和卸载看到了很好的结果，我们开始将其他东西从 CPU 转移到网络接口。TCP 分段卸载（TSO）是提高网络发送器性能的下一个伟大机制。我们可以形成一个模板，并将其与一大块数据一起发送到卡上，而不是为我们将要发送的 TCP 段形成 IP 数据包。网络接口在将数据包放置到线缆上时处理分段。TSO为 TCP 发送器带来了巨大的好处，使我们能够在耗尽单个核心之前就使10千兆网络接口饱和。
...
对于 UDP ，Linux 有一些通用机制，试图复制类似TSO的机制。此支持由通用分段卸载（GSO）和通用接收卸载（GRO）一起提供。GSO 在顺序上实现了巨大的改进，对于UDP发送器来说，可以提高20%，GRO更难测量，但机制是存在的。
...
**TJ：** 数据包批处理工作的背景是什么？
**JB：** 在接收时进行数据包批处理的想法已经存在了一段时间，至少是以我多次听到各种人提到的愿望列表项目的形式。我们已经有一些特定于TCP的用于发送（TSO）和接收（LRO）的数据包批处理形式。这种数据包批处理旨在比 LRO 更通用，以便它可以应用于其他协议（主要是UDP）。
**TJ：** 为什么需要做这项工作？
...
从John在这里的回应来看，还不清楚应该从哪里看到收益。iperf3 测量无法模拟非常繁忙的服务器的工作负载。对于 FreeBSD 中的数据包批处理来说，可能需要测试和调整更多的工作负载。通过关闭 John 的 github 分支并用您的网络流量实验，您可以帮助在 FreeBSD 中建立新的接收器优化。

---
**TOM JONES** 希望基于 FreeBSD 的项目得到应有的关注。他住在苏格兰东北部，提供 FreeBSD 咨询服务。
**JOHN BALDWIN** 是一名系统软件开发人员。20年来，他直接致力于在内核的各个部分（包括x86平台支持、SMP、各种设备驱动程序和虚拟内存子系统）和用户空间程序中对 FreeBSD 操作系统进行更改。除了编写代码，John 还在 FreeBSD 核心和发布工程团队中任职。他还为 GDB 调试器和 LLVM 做出了贡献。John 和他的妻子 Kimberly 以及三个孩子 Janelle、Evan 和 Bella 住在加利福尼亚州的康科德。
