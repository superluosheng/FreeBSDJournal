...
ZFS 将卷管理器和独立的文件系统的角色合二为一，与独立的文件系统相比具有多种优势。它以速度、灵活性而闻名，最重要的是，它非常注意防止数据丢失。许多传统的文件系统不得不一次只存在于一个磁盘上，而ZFS知道磁盘的底层结构，并创建一个可用的存储池，甚至在多个磁盘上。当额外的磁盘被添加到池中时，现有的文件系统将自动增长，立即成为文件系统的可用部分。
...
要卸载一个文件系统，使用 zfs umount，然后用 df 验证。
```
#  zfs  umount  example/compressed
#  df
```

验证 example/compressed没 是否作为挂载文件包含在输出下。该文件系统可以用 zfs重 新挂载。
```
...
ZFS 数据集的创建就像其他文件系统一样，下面的例子创建了一个名为data的新文件系统。
```
...
在创建了 zpool 之后，可以在该池中制作一个新的文件系统。
```
...
一个 RAID-Z 池是一个存储关键系统文件的好地方，例如用户的主目录。
```
...
由于大量的输入/输出要求，一次只能运行一次清理。擦除的长度取决于池中存储的数据量。清理完成后，使用 zpool status 查看状态。
```
#  zpool  status  storage
pool:  storage
state:  ONLINE
...
## ZFS管理
ZFS 有两个主要的管理实用程序：zpool实用程序控制池的操作，并允许添加、删除、替换和管理磁盘。zfs 实用程序允许创建、销毁和管理数据集，包括文件系统和卷。
虽然本入门指南不涉及 ZFS 管理，但您可以参考 zfs（8）和 zpool（8）了解其他 ZFS 选项。

## 其他资源
虽然使用本指南创建的非冗余池和 RAID-Z 池在大多数用例中都可以工作，但更复杂或更专业的系统可能需要进一步的 ZFS 管理和设置。ZFS 是一个功能强大且可自定义的文件系统，本指南几乎没有提及使用它可以做什么的表述。OpenZFS wiki 有大量关于安装、ZFS 系统管理和手册页面的文档。如果由于系统体系结构的原因需要进行调优，那么可以在 OpenZFS 和 FreeBSD 的 wiki 页面上找到 ZFS 调优指南。
